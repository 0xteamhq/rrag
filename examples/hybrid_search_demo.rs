...
use rrag::{
    ...
    BM25Config, TokenizerType, SemanticConfig, 
    RragResult, MockEmbeddingService
};
...
async fn main() -> RragResult<()> {
    ...
    let results = hybrid_retriever.search("How does meaning-based search work?", 3).await?;
    print_results(&results);
    ...
    let results = hybrid_retriever.search("phone with good camera and fast processor", 3).await?;
    print_results(&results);
    ...
    let results = hybrid_retriever.search("Rust implementation reciprocal rank fusion", 3).await?;
    print_results(&results);
    ...
    let results = hybrid_retriever.search("enviornmental carbon captur technology", 3).await?;
    print_results(&results);
    ...
    hybrid_retriever.record_feedback("BM25 algorithm IDF", 0.9).await?;
    hybrid_retriever.record_feedback("How does meaning-based search work?", 0.95).await?;
    hybrid_retriever.record_feedback("phone with good camera and fast processor", 0.85).await?;
    ...
    let results = hybrid_retriever.search(
    ...
    println!("   • BM25 Index: {} unique terms", 
        stats.bm25_stats.get("unique_terms").unwrap());
    println!("   • Semantic Index: {} documents", 
