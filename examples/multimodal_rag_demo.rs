...
use rrag::prelude::*;
use rrag::multimodal::{
    MultiModalService, MultiModalConfig, ProcessedImage, ExtractedTable, 
    AnalyzedChart, MultiModalDocument, ContentType
};
use std::path::Path;
...
async fn create_financial_report() -> RragResult<Path> {
    ...
    Ok(Path::new("./examples/data/financial_report.pdf"))
}

async fn create_research_paper() -> RragResult<Path> {
    ...
    Ok(Path::new("./examples/data/research_paper.pdf"))
}

async fn create_product_catalog() -> RragResult<Path> {
    ...
    Ok(Path::new("./examples/data/product_catalog.pdf"))
}
...
async fn demo_multimodal_search(documents: &[MultiModalDocument]) -> RragResult<()> {
    ...
    for (i, doc) in documents.iter().enumerate() {
        ...
        if relevance_score > 0.7 {
            println!("       Found in document {}: score {:.2}", i+1, relevance_score);
            
            // Show what modalities matched
            if doc.text_content.to_lowercase().contains("revenue") {
                println!("         ✓ Text content match");
            }
            if doc.charts.iter().any(|c| matches!(c.chart_type, ChartType::Line)) {
                println!("         ✓ Revenue trend chart detected");
            }
            if doc.tables.iter().any(|t| t.headers.iter().any(|h| h.contains("Revenue"))) {
