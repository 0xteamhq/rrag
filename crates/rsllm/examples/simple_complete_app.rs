...
fn main() -> Result<(), Box<dyn Error>> {
    println!("🚀 Simple Complete Application");
    println!("===============================\n");
    ...
    let r1 = registry.execute(&ToolCallExec::new("1", "calc", json!({"op": "mul", "a": 15, "b": 8})));
    println!("   Calculator: {}", r1.content);

    let r2 = registry.execute(&ToolCallExec::new("2", "tasks", json!({"action": "create", "desc": "Review PR"})));
    println!("   Tasks: {}", r2.content);

    let r3 = registry.execute(&ToolCallExec::new("3", "analyze", json!({"text": "Hello World!"})));
    println!("   Analyze: {}", r3.content);

    let r4 = registry.execute(&ToolCallExec::new("4", "convert", json!({"data": "test", "from": "json", "to": "csv"})));
    println!("   Convert: {}", r4.content);
    ...
    for r in registry.execute_batch(&batch) {
        println!("   {} → {}", r.tool_name, if r.success { "✅" } else { "❌" });
    }

    // Error handling
    println!("\n⚠️  Error Handling:\n");
    let err = registry.execute(&ToolCallExec::new("e1", "calc", json!({"op": "div", "a": 10, "b": 0})));
    println!("   Division by zero: {}", err.error.unwrap_or_default());

    println!("\n📊 Summary:");
    println!("   Total code per tool: ~15-20 lines");
    println!("   Total code for 4 tools: ~80 lines");
    println!("   vs Old approach: ~300 lines");
    println!("   Savings: 75% less code!");
