...
use crate::{ChatResponse, CompletionResponse, RsllmError, RsllmResult, StreamChunk};
...
impl StreamUtils {
    ...
    pub async fn buffer<S>(mut stream: S, max_size: usize) -> RsllmResult<Vec<StreamChunk>>
    where
...
pub trait RsllmStreamExt: Stream<Item = RsllmResult<StreamChunk>> + Sized {
    ...
    fn collect_chat_response(
        self,
    ) -> impl std::future::Future<Output = RsllmResult<ChatResponse>> + Send
    where
        Self: Send + Unpin,
    {
    ...
    fn collect_completion_response(
        self,
    ) -> impl std::future::Future<Output = RsllmResult<CompletionResponse>> + Send
    where
        Self: Send + Unpin,
    {
