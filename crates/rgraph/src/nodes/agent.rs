...
impl AgentNode {
    ...
    async fn generate_response(
        ...
        if self.should_use_tools(last_user_message) && !self.tools.is_empty() {
            ...
            Ok(AgentMessage {
                ...
                content: format!("I'll help you with that. Let me use the {} tool.", tool_name),
    ...
    fn should_use_tools(&self, input: &str) -> bool {
        ...
        tool_keywords.iter().any(|keyword| input_lower.contains(keyword))
    }
}
...
mod tests {
    ...
    async fn test_agent_node_with_tools() {
        ...
        let agent = AgentNode::new("test_agent", config)
            .with_tool("search".to_string(), tool);
