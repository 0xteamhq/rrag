...
impl HybridRetriever {
    ...
    pub fn new(
        ...
        let fusion: Arc<dyn RankFusion> = match &config.fusion_strategy {
            ...
            FusionStrategy::WeightedCombination => {
                Arc::new(WeightedFusion::new(vec![
    ...
    pub async fn search(
        ...
        let (bm25_results, semantic_results) = if self.config.parallel_retrieval {
            tokio::join!(
                ...
                self.semantic_retriever.search(query, limit * 2, Some(self.config.min_confidence))
            )
        } else {
            let bm25 = self.bm25_retriever.search(query, limit * 2).await;
            let semantic = self.semantic_retriever.search(query, limit * 2, Some(self.config.min_confidence)).await;
            (bm25, semantic)
        };
        ...
        let fused_results = match self.config.fusion_strategy {
            FusionStrategy::WeightedCombination => {
                let fusion = WeightedFusion::new(vec![
                    1.0 - semantic_weight,
                    semantic_weight,
                ]);
            ...
            _ => {
                self.fusion.fuse(vec![bm25_results, semantic_results], limit)?
            }
        };
    ...
    fn analyze_query(&self, query: &str) -> QueryCharacteristics {
        ...
        let has_entities = tokens.iter().any(|t| {
            t.chars().next().map_or(false, |c| c.is_uppercase())
        });
        
        // Technical term detection (simplified)
        let technical_terms = ["algorithm", "function", "method", "system", "protocol", "framework"];
        let has_technical_terms = tokens.iter().any(|t| {
            technical_terms.contains(&t.to_lowercase().as_str())
        });
    ...
    async fn calculate_adaptive_weight(&self, characteristics: &QueryCharacteristics) -> f32 {
        ...
        if history.len() > 10 {
            ...
            let recent_weights: Vec<f32> = history.iter()
                .rev()
                .take(10)
...
mod tests {
    ...
    async fn test_hybrid_search() {
        ...
        retriever.record_feedback("machine learning AI", 0.9).await.unwrap();
    }
}