...
pub trait QueryExpander: Send + Sync {
    ...
    async fn expand_query(&self, query: &str, options: &ExpansionOptions) -> RragResult<ExpansionResult>;
    
    /// Expand query terms using graph structure
    async fn expand_terms(&self, terms: &[String], options: &ExpansionOptions) -> RragResult<Vec<String>>;
    
    /// Find related entities for query expansion
    async fn find_related_entities(&self, entities: &[String], options: &ExpansionOptions) -> RragResult<Vec<String>>;
    
    /// Get expansion suggestions for a query
    async fn get_suggestions(&self, query: &str, max_suggestions: usize) -> RragResult<Vec<String>>;
}
...
impl GraphQueryExpander {
    ...
    async fn semantic_expansion(&self, entity_ids: &[String], options: &ExpansionOptions) -> RragResult<Vec<ExpandedTerm>> {
    ...
    async fn hierarchical_expansion(&self, entity_ids: &[String], options: &ExpansionOptions) -> RragResult<Vec<ExpandedTerm>> {
        let mut expanded_terms = Vec::new();
        let strategy_weight = self.config.strategy_weights.get(&ExpansionStrategy::Hierarchical).copied().unwrap_or(0.8);
        
        for entity_id in entity_ids {
            // Find hierarchical edges (parent/child relationships)
            let hierarchical_edges: Vec<_> = self.graph.edges.values()
                .filter(|edge| {
                    (edge.source_id == *entity_id || edge.target_id == *entity_id) &&
                ...
                if let Some(related_node) = self.graph.get_node(related_node_id) {
                    ...
                    if confidence >= options.min_confidence {
                        let expanded_term = ExpandedTerm {
                            ...
                            source_entities: vec![entity_id.clone()],
                            relationship: Some(
                                if edge.source_id == *entity_id { "parent".to_string() } 
    ...
    async fn similarity_expansion(&self, entity_ids: &[String], options: &ExpansionOptions) -> RragResult<Vec<ExpandedTerm>> {
        let mut expanded_terms = Vec::new();
        let strategy_weight = self.config.strategy_weights.get(&ExpansionStrategy::Similarity).copied().unwrap_or(0.7);
        ...
        for entity_id in entity_ids {
            if let Some(entity_node) = self.graph.get_node(entity_id) {
                ...
                if let Some(entity_embedding) = &entity_node.embedding {
                    for (other_id, other_node) in &self.graph.nodes {
                        ...
                        if let Some(other_embedding) = &other_node.embedding {
                            if let Ok(similarity) = entity_embedding.cosine_similarity(other_embedding) {
                                if similarity >= self.config.min_similarity_threshold {
                                    ...
                                    if confidence >= options.min_confidence {
                                        let expanded_term = ExpandedTerm {
                                            ...
                                            source_entities: vec![entity_id.clone()],
                                            relationship: Some(format!("similarity:{:.2}", similarity)),
                                        };
                                        expanded_terms.push(expanded_term);
                                    }
                                }
                ...
                let similarity_edges: Vec<_> = self.graph.edges.values()
                    .filter(|edge| {
                        (edge.source_id == *entity_id || edge.target_id == *entity_id) &&
    ...
    async fn cooccurrence_expansion(&self, entity_ids: &[String], options: &ExpansionOptions) -> RragResult<Vec<ExpandedTerm>> {
        let mut expanded_terms = Vec::new();
        let strategy_weight = self.config.strategy_weights.get(&ExpansionStrategy::CoOccurrence).copied().unwrap_or(0.6);
        
        for entity_id in entity_ids {
            // Find co-occurrence edges
            let cooccurrence_edges: Vec<_> = self.graph.edges.values()
                .filter(|edge| {
                    (edge.source_id == *entity_id || edge.target_id == *entity_id) &&
    ...
    async fn entity_type_expansion(&self, entity_ids: &[String], options: &ExpansionOptions) -> RragResult<Vec<ExpandedTerm>> {
        let mut expanded_terms = Vec::new();
        let strategy_weight = self.config.strategy_weights.get(&ExpansionStrategy::EntityType).copied().unwrap_or(0.5);
        ...
        for entity_id in entity_ids {
            if let Some(entity_node) = self.graph.get_node(entity_id) {
                ...
                entities_by_type.entry(type_key).or_default().push(entity_id.clone());
            }
        }
        
        // For each type, find other entities of the same type
        for (entity_type, type_entities) in entities_by_type {
            let similar_type_nodes: Vec<_> = self.graph.nodes.values()
                .filter(|node| {
            ...
            for node in similar_type_nodes.into_iter().take(5) { // Limit to avoid too many results
    ...
    async fn path_based_expansion(&self, entity_ids: &[String], options: &ExpansionOptions) -> RragResult<Vec<ExpandedTerm>> {
        let mut expanded_terms = Vec::new();
        let strategy_weight = self.config.strategy_weights.get(&ExpansionStrategy::PathBased).copied().unwrap_or(0.4);
        ...
        for entity_id in entity_ids {
            ...
            if let Ok(visited_nodes) = GraphAlgorithms::bfs_search(&self.graph, entity_id, &traversal_config) {
                for visited_node_id in visited_nodes.iter().skip(1) { // Skip the source node
                    if let Some(visited_node) = self.graph.get_node(visited_node_id) {
                        // Calculate confidence based on distance from source
                        let distance = visited_nodes.iter().position(|id| id == visited_node_id).unwrap_or(0);
    ...
    fn apply_focus_boosting(&self, terms: &mut [ExpandedTerm], focus_entities: &[String]) {
        ...
        for term in terms {
            ...
            let is_related = term.source_entities.iter()
...
impl QueryExpander for GraphQueryExpander {
    async fn expand_query(&self, query: &str, options: &ExpansionOptions) -> RragResult<ExpansionResult> {
        ...
        if self.config.enable_caching {
            ...
            if let Some(cached_terms) = self.expansion_cache.read().await.get(&cache_key) {
                let result = ExpansionResult {
                    ...
                    expanded_terms: cached_terms.iter().map(|term| ExpandedTerm {
                        term: term.clone(),
        ...
        if options.include_original {
            let original_terms: Vec<_> = query.split_whitespace()
        ...
        if self.config.enable_caching {
            ...
            self.expansion_cache.write().await.insert(cache_key, cache_terms);
        }
    ...
    async fn expand_terms(&self, terms: &[String], options: &ExpansionOptions) -> RragResult<Vec<String>> {
        ...
        Ok(expansion_result.expanded_terms.into_iter().map(|t| t.term).collect())
    }

    async fn find_related_entities(&self, entities: &[String], options: &ExpansionOptions) -> RragResult<Vec<String>> {
        // Find entity IDs matching the given entity names
        let entity_ids: Vec<_> = entities.iter()
...
mod tests {
    ...
    fn create_test_graph() -> KnowledgeGraph {
        ...
        graph.add_edge(GraphEdge::new(
            node3_id.clone(), node1_id.clone(), "is_a", 
            EdgeType::Semantic("is_a".to_string())
        ).with_confidence(0.9).with_weight(0.9)).unwrap();
        
        graph.add_edge(GraphEdge::new(
            node1_id.clone(), node2_id.clone(), "part_of", 
            EdgeType::Semantic("part_of".to_string())
        ).with_confidence(0.8).with_weight(0.8)).unwrap();
        
        graph.add_edge(GraphEdge::new(
            node4_id.clone(), node3_id.clone(), "used_in", 
            EdgeType::Semantic("used_in".to_string())
        ).with_confidence(0.7).with_weight(0.7)).unwrap();
    ...
    async fn test_query_expansion() {
        ...
        let result = expander.expand_query("machine learning", &options).await.unwrap();
    ...
    async fn test_semantic_expansion() {
        ...
        let ml_node_id = graph.nodes.values()
        ...
        let expanded_terms = expander.semantic_expansion(&[ml_node_id], &options).await.unwrap();
        ...
        assert!(term_texts.contains(&&"artificial intelligence".to_string()) || 
    ...
    async fn test_term_expansion() {
        ...
        let options = ExpansionOptions::default();
        let expanded_terms = expander.expand_terms(&["machine learning".to_string()], &options).await.unwrap();
