...
use super::{EdgeType, GraphEdge, GraphError, GraphNode, NodeType};
...
pub trait EntityExtractor: Send + Sync {
    ...
    async fn extract_all(
        ...
        let relationships = self
            .extract_relationships(text, &entities, source_id)
            .await?;
...
impl RuleBasedEntityExtractor {
    ...
    fn compile_entity_patterns(
        config: &EntityExtractionConfig,
    ) -> RragResult<HashMap<EntityType, Vec<Regex>>> {
        ...
        let person_patterns = vec![
            ...
            Regex::new(r"\b(?:Mr|Mrs|Dr|Prof|Ms)\.?\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b").map_err(
                |e| GraphError::EntityExtraction {
        ...
        let money_patterns = vec![
            ...
            Regex::new(r"\b\d+(?:,\d{3})*(?:\.\d{2})?\s*(?:dollars?|USD|cents?)\b").map_err(
                |e| GraphError::EntityExtraction {
        ...
        let percentage_patterns = vec![
            Regex::new(r"\b\d+(?:\.\d+)?%\b").map_err(|e| GraphError::EntityExtraction {
    ...
    fn compile_relationship_patterns() -> RragResult<HashMap<RelationType, Vec<Regex>>> {
        ...
        let is_a_patterns = vec![
            Regex::new(r"(.+?)\s+is\s+a\s+(.+?)").map_err(|e| GraphError::EntityExtraction {
        ...
        let located_in_patterns = vec![
            ...
            Regex::new(r"(.+?)\s+in\s+(.+?)").map_err(|e| GraphError::EntityExtraction {
        ...
        let owns_patterns = vec![
            Regex::new(r"(.+?)\s+owns\s+(.+?)").map_err(|e| GraphError::EntityExtraction {
    ...
    fn is_stop_word_only(&self, text: &str) -> bool {
        ...
        words
            .iter()
            .all(|word| self.config.stop_words.contains(&word.to_lowercase()))
    }
    ...
    fn extract_relationships_with_patterns(
        ...
        let mut entity_spans: Vec<(usize, usize, &Entity)> = entities
            .iter()
            ...
            .find(|(_, _, entity)| {
                entity.text.eq_ignore_ascii_case(text)
                    || entity
                        .normalized_form
                        .as_ref()
                        .map_or(false, |norm| norm.eq_ignore_ascii_case(text))
            })
                ...
                if distance <= max_distance {
                    ...
                    if confidence >= self.config.min_confidence {
                        let relationship = Relationship {
                            ...
                            context: format!("Co-occurrence within {} characters", distance),
                            ...
                            attributes: {
                                ...
                                attrs.insert(
                                    "distance".to_string(),
                                    serde_json::Value::Number(distance.into()),
                                );
                                attrs.insert(
                                    "type".to_string(),
...
pub fn entities_to_nodes(entities: &[Entity]) -> Vec<GraphNode> {
    entities
        ...
        .map(|entity| {
            ...
            node = node.with_attribute(
                "start_pos",
                serde_json::Value::Number(entity.start_pos.into()),
            );
...
pub fn relationships_to_edges(
    relationships: &[Relationship],
    entity_node_map: &HashMap<String, String>,
) -> Vec<GraphEdge> {
    relationships
        .iter()
        .filter_map(|relationship| {
            ...
            edge = edge.with_attribute(
                "context",
                serde_json::Value::String(relationship.context.clone()),
            );
            edge = edge.with_attribute(
                "start_pos",
                serde_json::Value::Number(relationship.start_pos.into()),
            );
            edge = edge.with_attribute(
                "end_pos",
                serde_json::Value::Number(relationship.end_pos.into()),
            );
...
mod tests {
    ...
    async fn test_relationship_extraction() {
        ...
        let work_relations: Vec<_> = relationships
            .iter()
