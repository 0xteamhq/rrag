...
impl KnowledgeGraph {
    ...
    pub fn add_node(&mut self, node: GraphNode) -> RragResult<()> {
        ...
        self.adjacency_list.entry(node_id.clone()).or_insert_with(HashSet::new);
        self.reverse_adjacency_list.entry(node_id.clone()).or_insert_with(HashSet::new);
    ...
    pub fn add_edge(&mut self, edge: GraphEdge) -> RragResult<()> {
        ...
        if !self.nodes.contains_key(&source_id) {
            return Err(crate::RragError::retrieval(
    ...
    pub fn calculate_metrics(&self) -> GraphMetrics {
        ...
        let degrees: Vec<usize> = self.adjacency_list
            .values()
...
mod tests {
    ...
    fn test_get_neighbors() {
        ...
        graph.add_edge(GraphEdge::new(
            node1_id.clone(), node2_id.clone(), "connected", EdgeType::Similar
        )).unwrap();
        
        graph.add_edge(GraphEdge::new(
            node1_id.clone(), node3_id.clone(), "connected", EdgeType::Similar
        )).unwrap();
