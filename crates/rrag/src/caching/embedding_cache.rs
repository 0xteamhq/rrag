...
impl EmbeddingCache {
    ...
    fn compress_embedding(&self, embedding: &[f32]) -> CompressedEmbedding {
        ...
        let (min_val, max_val) = embedding.iter()
        ...
        let quantized_values: Vec<u8> = embedding.iter()
            .map(|&val| {
        ...
        let compressed_size = quantized_values.len() * std::mem::size_of::<u8>() 
            + std::mem::size_of::<f32>() * 2; // scale + offset
    ...
    fn decompress_embedding(&self, compressed: &CompressedEmbedding) -> Vec<f32> {
        compressed.quantized_values.iter()
    ...
    pub fn get_compression_stats(&self) -> Option<CompressionStats> {
        ...
        for compressed in compressed_storage.values() {
            ...
            let compressed_size = compressed.quantized_values.len() + 
                std::mem::size_of::<f32>() * 2; // scale + offset
...
impl Cache<String, EmbeddingCacheEntry> for EmbeddingCache {
    ...
    fn contains(&self, key: &String) -> bool {
        ...
        (self.compressed_storage.as_ref().map_or(false, |cs| cs.contains_key(key)))
    }
...
mod tests {
    ...
    fn test_basic_operations() {
        ...
        // Cache embedding
        cache.cache_embedding(text.clone(), model.clone(), embedding.clone()).unwrap();
    ...
    fn test_deduplication() {
        ...
        cache.cache_embedding(text.clone(), "model1".to_string(), embedding.clone()).unwrap();
        cache.cache_embedding(text.clone(), "model2".to_string(), embedding.clone()).unwrap();
