...
impl ResultCache {
    ...
    pub fn cache_results(
        ...
        let should_compress = self.config.compress_large_results && 
                            results.len() > 100;
        
        if should_compress {
            let compressed = self.compress_results(&results);
            self.compressed_storage.insert(params_hash.clone(), compressed);
        } else {
    ...
    pub fn invalidate_pattern(&mut self, pattern: &str) {
        let keys_to_remove: Vec<String> = self.storage.keys()
    ...
    pub fn get_insights(&self) -> ResultCacheInsights {
        ...
        let avg_results_per_entry = if !self.storage.is_empty() {
            self.storage.values()
                .map(|e| e.results.len())
                .sum::<usize>() as f32 / self.storage.len() as f32
        } else {
            0.0
        };
        
        let compression_ratio = if !self.compressed_storage.is_empty() {
            let total_original: usize = self.compressed_storage.values()
                .map(|c| c.original_size)
                .sum();
            let total_compressed: usize = self.compressed_storage.values()
                .map(|c| c.data.len())
                .sum();
    ...
    fn estimate_memory_usage(&self) -> usize {
        let storage_size: usize = self.storage.values()
            .map(|e| {
        ...
        let compressed_size: usize = self.compressed_storage.values()
...
mod tests {
    ...
    fn create_test_results(count: usize) -> Vec<CachedSearchResult> {
        (0..count).map(|i| CachedSearchResult {
    ...
    fn test_basic_caching() {
        ...
        cache.cache_results(params_hash.clone(), results.clone(), metadata).unwrap();
    ...
    fn test_invalidation() {
        ...
        cache.cache_results("user_123".to_string(), results.clone(), metadata.clone()).unwrap();
        cache.cache_results("user_456".to_string(), results.clone(), metadata.clone()).unwrap();
        cache.cache_results("product_789".to_string(), results.clone(), metadata).unwrap();
    ...
    fn test_insights() {
        ...
        cache.cache_results("small".to_string(), results_small, metadata.clone()).unwrap();
        cache.cache_results("large".to_string(), results_large, metadata).unwrap();
