...
impl EvaluationService {
    ...
    fn initialize_evaluators(&mut self) {
        for eval_type in &self.config.enabled_evaluations {
            let evaluator: Box<dyn Evaluator> = match eval_type {
                EvaluationType::Ragas => Box::new(ragas::RagasEvaluator::new(self.config.ragas_config.clone())),
                EvaluationType::Retrieval => Box::new(retrieval_eval::RetrievalEvaluator::new(self.config.retrieval_config.clone())),
                EvaluationType::Generation => Box::new(generation_eval::GenerationEvaluator::new(self.config.generation_config.clone())),
                EvaluationType::EndToEnd => Box::new(end_to_end::EndToEndEvaluator::new(self.config.e2e_config.clone())),
    ...
    pub async fn evaluate(&mut self, data: EvaluationData) -> RragResult<HashMap<EvaluationType, EvaluationResult>> {
            ...
            match evaluator.evaluate(&data) {
                Ok(result) => {
                    ...
                    self.metrics_collector.record_metric(
                        "evaluation_time_ms",
                        eval_time,
                        Some(&[("evaluator".to_string(), evaluator.name().to_string())].iter().cloned().collect())
                    )?;
                    
                    results.insert(eval_type.clone(), result);
                    println!("✅ {} evaluation completed in {:.2}ms", evaluator.name(), eval_time);
                }
                Err(e) => {
        ...
        self.metrics_collector.record_metric("total_evaluation_time_ms", total_time, None)?;
    ...
    pub async fn export_results(&self, results: &HashMap<EvaluationType, EvaluationResult>) -> RragResult<()> {
        ...
        std::fs::create_dir_all(&self.config.output_config.output_dir)
            .map_err(|e| RragError::evaluation(format!("Failed to create output directory: {}", e)))?;
    ...
    async fn export_json(&self, results: &HashMap<EvaluationType, EvaluationResult>) -> RragResult<()> {
        let json_path = format!("{}/evaluation_results.json", self.config.output_config.output_dir);
    ...
    async fn export_csv(&self, results: &HashMap<EvaluationType, EvaluationResult>) -> RragResult<()> {
        let csv_path = format!("{}/evaluation_summary.csv", self.config.output_config.output_dir);
    ...
    async fn export_html(&self, results: &HashMap<EvaluationType, EvaluationResult>) -> RragResult<()> {
        let html_path = format!("{}/evaluation_report.html", self.config.output_config.output_dir);
...
<body>
    <div class="header">
        ...
        html_content.push_str(&chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC").to_string());
    ...
    <div class="evaluator">
        ...
        <table>
            <tr><th>Metric</th><th>Score</th></tr>"#, eval_type));
    ...
    async fn export_markdown(&self, results: &HashMap<EvaluationType, EvaluationResult>) -> RragResult<()> {
        let md_path = format!("{}/evaluation_report.md", self.config.output_config.output_dir);
...
impl MetricsCollector for DefaultMetricsCollector {
    ...
    fn record_metric(&mut self, name: &str, value: f32, labels: Option<&HashMap<String, String>>) -> RragResult<()> {
        ...
        self.metrics.entry(name.to_string()).or_insert_with(Vec::new).push(record);
        Ok(())
    }
    ...
    fn export_metrics(&self, format: &ExportFormat, output_path: &str) -> RragResult<()> {
        match format {
            ExportFormat::Json => {
                let json_content = serde_json::to_string_pretty(&self.metrics)
                    .map_err(|e| RragError::evaluation(format!("Failed to serialize metrics: {}", e)))?;
                std::fs::write(output_path, json_content)
                    .map_err(|e| RragError::evaluation(format!("Failed to write metrics file: {}", e)))?;
            }
            _ => {
                return Err(RragError::evaluation("Unsupported export format for metrics".to_string()));
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_evaluation_config_creation() {
        ...
        assert!(config.enabled_evaluations.contains(&EvaluationType::Retrieval));
        assert!(config.enabled_evaluations.contains(&EvaluationType::Generation));
    }
