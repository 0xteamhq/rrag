...
impl VersionManager {
    ...
    pub async fn create_version(
        ...
        let history = histories.entry(document.id.clone()).or_insert_with(|| {
            VersionHistory {
        ...
        let change_summary = if self.config.enable_change_tracking {
            self.compute_change_summary(document, &parent_version, change_type.clone(), history).await?
        } else {
            None
        };
        ...
        if let Some(parent) = &parent_version {
            history.version_tree.entry(parent.clone()).or_insert_with(Vec::new).push(version_id.clone());
        }
        ...
        // Update statistics
        {
            ...
            stats.avg_versions_per_document = stats.total_versions as f64 / stats.total_documents as f64;
    ...
    pub async fn get_current_version(&self, document_id: &str) -> RragResult<Option<DocumentVersion>> {
        ...
        if let Some(history) = histories.get(document_id) {
            return self.get_version(document_id, &history.current_version).await;
        }
            ...
            if let Some(expected) = expected_version {
                if history.current_version != expected {
                    ...
                    let conflict = VersionConflict {
                        conflict_id,
                        document_id: document.id.clone(),
                        conflicting_versions: vec![history.current_version.clone(), expected.to_string()],
    ...
    pub async fn resolve_conflict(
        ...
        if let Some(conflict) = conflicts.get_mut(conflict_id) {
            ...
            // Update statistics
            {
                ...
                stats.unresolved_conflicts = conflicts.values()
    ...
    async fn start_cleanup_task(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let cutoff_date = chrono::Utc::now() - chrono::Duration::days(config.retention_period_days as i64);
            ...
            loop {
                ...
                for conflict in conflicts_guard.values_mut() {
                    if conflict.resolution_status == ResolutionStatus::Unresolved {
                        ...
                        if let Some(auto_resolution) = Self::apply_auto_resolution(conflict, &config.default_resolution).await {
                ...
                if resolved_count > 0 {
                    ...
                    stats_guard.unresolved_conflicts = conflicts_guard.values()
                        .filter(|c| c.resolution_status == ResolutionStatus::Unresolved)
                        .count();
                }
            }
    ...
    async fn compute_change_summary(
        ...
        if let Some(parent_id) = parent_version_id {
            ...
            if let Some(parent_version) = history.versions.iter().find(|v| v.version_id == *parent_id) {
...
mod tests {
    ...
    async fn test_create_version() {
        let manager = VersionManager::new(VersioningConfig::default()).await.unwrap();
        let doc = Document::new("Test content");
        
        let version = manager.create_version(&doc, "test_author", ChangeType::Initial, None).await.unwrap();
    ...
    async fn test_version_retrieval() {
        ...
        // Test get_version
        let retrieved = manager.get_version(&doc.id, &version.version_id).await.unwrap().unwrap();
    ...
    async fn test_multiple_versions() {
        ...
        let version1 = manager.create_version(&doc1, "author1", ChangeType::Initial, None).await.unwrap();
        assert_eq!(version1.version_number, 1);
        
        // Create second version
        let version2 = manager.create_version(&doc2, "author2", ChangeType::Major, None).await.unwrap();
        assert_eq!(version2.version_number, 2);
        
        // Check current version is the latest
        let current = manager.get_current_version("test_doc").await.unwrap().unwrap();
        assert_eq!(current.version_id, version2.version_id);
        
        // Check version history
        let history = manager.get_version_history("test_doc").await.unwrap().unwrap();
    ...
    async fn test_conflict_detection() {
        ...
        let version1 = manager.create_version(&doc, "author1", ChangeType::Initial, None).await.unwrap();
        
        // Try to update with wrong expected version
        let conflict = manager.detect_conflicts(&doc, Some("wrong_version_id")).await.unwrap();
        ...
        let no_conflict = manager.detect_conflicts(&doc, Some(&version1.version_id)).await.unwrap();
