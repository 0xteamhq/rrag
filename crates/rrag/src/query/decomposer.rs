...
impl QueryDecomposer {
    ...
    pub async fn decompose(&self, query: &str) -> RragResult<Vec<SubQuery>> {
        ...
        sub_queries.sort_by(|a, b| b.priority.partial_cmp(&a.priority).unwrap_or(std::cmp::Ordering::Equal));
    ...
    fn should_decompose(&self, query: &str) -> bool {
        ...
        let has_complexity_indicators = self.complexity_indicators
            .iter()
    ...
    fn temporal_decomposition(&self, query: &str) -> Vec<SubQuery> {
        ...
        // Time indicators
        let time_indicators = ["when", "before", "after", "during", "since", "until", "timeline"];
        
        if time_indicators.iter().any(|&indicator| query_lower.contains(indicator)) {
    ...
    fn comparative_decomposition(&self, query: &str) -> Vec<SubQuery> {
        ...
        let comparison_indicators = ["vs", "versus", "compare", "difference", "similar", "different"];
        
        if comparison_indicators.iter().any(|&indicator| query_lower.contains(indicator)) {
    ...
    fn complete_sub_query(&self, partial: &str) -> String {
        ...
        let starts_with_question = question_words.iter()
    ...
    fn extract_temporal_aspects(&self, query: &str) -> Vec<String> {
        ...
        if query.to_lowercase().contains("when") {
            aspects.push(format!("When did {} happen?", self.extract_main_subject(query)));
        }
        
        if query.to_lowercase().contains("before") {
            aspects.push(format!("What happened before {}?", self.extract_main_subject(query)));
        }
        
        if query.to_lowercase().contains("after") {
            aspects.push(format!("What happened after {}?", self.extract_main_subject(query)));
        }
    ...
    fn extract_comparison_items(&self, query: &str) -> Vec<String> {
        ...
        if let Some(vs_pos) = query.to_lowercase().find(" vs ") {
            ...
            for word in words {
                ...
                if collecting && word.len() > 2 && !["and", "with", "to"].contains(&word.to_lowercase().as_str()) {
                    items.push(word.trim_matches(|c: char| !c.is_alphanumeric()).to_string());
                    if items.len() >= 2 {
                        break;
                    }
                }
    ...
    fn extract_main_subject(&self, query: &str) -> String {
        ...
        for word in words {
            if word.len() > 3 && !["what", "when", "where", "how", "why", "who", "the", "and"].contains(&word.to_lowercase().as_str()) {
    ...
    fn extract_last_noun<'a>(&self, text: &'a str) -> &'a str {
        ...
        for word in words.iter().rev() {
            if word.len() > 2 && !["the", "and", "or", "of", "in", "on", "at"].contains(&word.to_lowercase().as_str()) {
                return word;
            }
        }
        text
    ...
    fn init_patterns() -> Vec<DecompositionPattern> {
        vec![
            DecompositionPattern {
                ...
                triggers: vec!["and".to_string(), "also".to_string(), "additionally".to_string()],
                strategy: DecompositionStrategy::Logical,
                extractor: |query| {
                    query.split(" and ")
            ...
            DecompositionPattern {
                ...
                triggers: vec!["vs".to_string(), "compare".to_string(), "difference".to_string()],
                strategy: DecompositionStrategy::Comparative,
                extractor: |query| {
...
mod tests {
    ...
    async fn test_logical_decomposition() {
        ...
        assert!(sub_queries.iter().any(|sq| sq.strategy == DecompositionStrategy::Logical));
    }
    ...
    async fn test_comparative_decomposition() {
        ...
        assert!(!sub_queries.is_empty());
        let comparative_queries: Vec<_> = sub_queries.iter()
    ...
    async fn test_temporal_decomposition() {
        ...
        assert!(!sub_queries.is_empty());
        let temporal_queries: Vec<_> = sub_queries.iter()
