...
impl QueryClassifier {
    ...
    fn detect_intent(&self, query: &str, tokens: &[String]) -> (QueryIntent, f32) {
        ...
        for pattern in &self.patterns {
            ...
            if matches > 0 {
                // Normalize score
                let normalized_score = (score / (pattern.keywords.len() + pattern.patterns.len()) as f32) * pattern.confidence;
        ...
        if best_confidence < 0.3 {
            if query.starts_with("what is") || query.starts_with("define") {
                ...
                best_confidence = 0.6;
            } else if query.contains("compare") || query.contains("vs") || query.contains("difference") {
    ...
    fn detect_query_type(&self, query: &str, tokens: &[String]) -> (QueryType, f32) {
        ...
        for pattern in &self.type_patterns {
            ...
            if matches > 0 {
                let confidence = (matches as f32 / pattern.indicators.len() as f32) * pattern.confidence;
    ...
    fn extract_entities(&self, tokens: &[String]) -> Vec<String> {
        ...
        for token in tokens {
            ...
            let tech_terms = ["api", "sql", "json", "html", "css", "javascript", "python", "rust", "docker"];
    ...
    fn detect_domain(&self, tokens: &[String]) -> Option<String> {
        let domains = [
            ("technology", vec!["code", "programming", "software", "api", "database", "algorithm", "computer"]),
            ("science", vec!["research", "study", "experiment", "theory", "analysis", "data", "scientific"]),
            ("business", vec!["market", "sales", "revenue", "customer", "profit", "strategy", "management"]),
            ("health", vec!["medical", "health", "disease", "treatment", "doctor", "medicine", "patient"]),
            ("education", vec!["learn", "study", "school", "university", "course", "education", "teach"]),
        ];
    ...
    fn needs_context(&self, _query: &str, tokens: &[String]) -> bool {
        let context_indicators = ["this", "that", "it", "they", "them", "previous", "above", "following"];
        let pronouns = ["it", "this", "that", "these", "those"];
        
        // Check for pronouns without clear antecedents
        let has_pronouns = pronouns.iter().any(|&pronoun| tokens.contains(&pronoun.to_string()));
        
        // Check for context indicators
        let has_context_indicators = context_indicators.iter()
    ...
    fn suggest_strategies(&self, intent: &QueryIntent, query_type: &QueryType, complexity: f32) -> Vec<String> {
    ...
    fn init_intent_patterns() -> Vec<IntentPattern> {
        vec![
            IntentPattern {
                ...
                keywords: vec!["define".to_string(), "definition".to_string(), "meaning".to_string()],
                patterns: vec!["what is".to_string(), "what does".to_string(), "define".to_string()],
                confidence: 0.9,
            },
            IntentPattern {
                intent: QueryIntent::Procedural,
                keywords: vec!["how".to_string(), "step".to_string(), "tutorial".to_string(), "guide".to_string()],
                patterns: vec!["how to".to_string(), "step by step".to_string(), "how do i".to_string()],
                confidence: 0.9,
            },
            IntentPattern {
                intent: QueryIntent::Comparative,
                keywords: vec!["compare".to_string(), "difference".to_string(), "better".to_string(), "versus".to_string()],
                patterns: vec!["vs".to_string(), "compared to".to_string(), "difference between".to_string()],
                confidence: 0.8,
            },
            IntentPattern {
                intent: QueryIntent::Troubleshooting,
                keywords: vec!["problem".to_string(), "error".to_string(), "fix".to_string(), "issue".to_string(), "broken".to_string()],
                patterns: vec!["not working".to_string(), "how to fix".to_string(), "troubleshoot".to_string()],
                confidence: 0.8,
            },
            IntentPattern {
                intent: QueryIntent::Factual,
                keywords: vec!["when".to_string(), "where".to_string(), "who".to_string(), "which".to_string()],
                patterns: vec!["when did".to_string(), "where is".to_string(), "who created".to_string()],
                confidence: 0.7,
            },
        ]
    }
    ...
    fn init_type_patterns() -> Vec<TypePattern> {
        vec![
            TypePattern {
                ...
                indicators: vec!["?".to_string(), "what".to_string(), "how".to_string(), "why".to_string(), "when".to_string(), "where".to_string()],
                confidence: 0.9,
            },
            TypePattern {
                query_type: QueryType::Command,
                indicators: vec!["show".to_string(), "find".to_string(), "get".to_string(), "list".to_string(), "give".to_string()],
                confidence: 0.8,
            },
            TypePattern {
                query_type: QueryType::Complex,
                indicators: vec!["and".to_string(), "or".to_string(), "but".to_string(), "however".to_string(), "also".to_string()],
                confidence: 0.7,
            },
        ]
    }
...
mod tests {
    ...
    async fn test_definitional_query() {
        ...
        let result = classifier.classify("What is machine learning?").await.unwrap();
    ...
    async fn test_procedural_query() {
        ...
        let result = classifier.classify("How to implement a REST API?").await.unwrap();
    ...
    async fn test_comparative_query() {
        ...
        let result = classifier.classify("Python vs Rust performance comparison").await.unwrap();
