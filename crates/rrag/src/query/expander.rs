...
impl QueryExpander {
    ...
    pub async fn expand(&self, query: &str) -> RragResult<Vec<ExpansionResult>> {
        ...
        if self.config.enable_domain_expansion {
            ...
            results.extend(domain_results.into_iter()
        ...
        for token in tokens {
            if let Some(synonyms) = self.synonyms.get(&token.to_lowercase()) {
                for synonym in synonyms.iter().take(self.config.max_synonyms) {
                    if !tokens.iter().any(|t| t.to_lowercase() == synonym.to_lowercase()) {
        ...
        for token in tokens {
            if let Some(related) = self.related_terms.get(&token.to_lowercase()) {
                for term in related.iter().take(self.config.max_related_terms) {
                    if !tokens.iter().any(|t| t.to_lowercase() == term.to_lowercase()) {
    ...
    fn detect_domain(&self, tokens: &[String]) -> String {
        let tech_terms = ["code", "programming", "software", "api", "database", "algorithm"];
        ...
        let tech_count = tech_terms.iter().filter(|&&term| 
            tokens_lower.iter().any(|t| t.contains(term))).count();
        let business_count = business_terms.iter().filter(|&&term| 
            tokens_lower.iter().any(|t| t.contains(term))).count();
        let science_count = science_terms.iter().filter(|&&term| 
            tokens_lower.iter().any(|t| t.contains(term))).count();
    ...
    fn init_synonyms() -> HashMap<String, Vec<String>> {
        ...
        synonyms.insert("small".to_string(), vec!["tiny".to_string(), "little".to_string(), "compact".to_string()]);
        synonyms.insert("good".to_string(), vec!["excellent".to_string(), "great".to_string(), "quality".to_string()]);
        synonyms.insert("bad".to_string(), vec!["poor".to_string(), "terrible".to_string(), "awful".to_string()]);
        synonyms.insert("simple".to_string(), vec!["easy".to_string(), "basic".to_string(), "straightforward".to_string()]);
        synonyms.insert("difficult".to_string(), vec!["hard".to_string(), "challenging".to_string(), "complex".to_string()]);
        synonyms.insert("method".to_string(), vec!["approach".to_string(), "technique".to_string(), "way".to_string()]);
        synonyms.insert("create".to_string(), vec!["build".to_string(), "make".to_string(), "develop".to_string()]);
        synonyms.insert("use".to_string(), vec!["utilize".to_string(), "employ".to_string(), "apply".to_string()]);
    ...
    fn init_related_terms() -> HashMap<String, Vec<String>> {
        ...
        related.insert("programming".to_string(), vec!["coding".to_string(), "development".to_string(), "software".to_string()]);
        related.insert("database".to_string(), vec!["data".to_string(), "storage".to_string(), "query".to_string()]);
        related.insert("algorithm".to_string(), vec!["logic".to_string(), "computation".to_string(), "optimization".to_string()]);
        related.insert("machine".to_string(), vec!["learning".to_string(), "ai".to_string(), "model".to_string()]);
        related.insert("web".to_string(), vec!["website".to_string(), "internet".to_string(), "browser".to_string()]);
        related.insert("api".to_string(), vec!["interface".to_string(), "endpoint".to_string(), "service".to_string()]);
        related.insert("security".to_string(), vec!["encryption".to_string(), "authentication".to_string(), "protection".to_string()]);
        related.insert("performance".to_string(), vec!["speed".to_string(), "optimization".to_string(), "efficiency".to_string()]);
    ...
    fn init_domain_expansions() -> HashMap<String, HashMap<String, Vec<String>>> {
        ...
        tech_expansions.insert("ml".to_string(), vec!["machine learning".to_string(), "artificial intelligence".to_string()]);
        tech_expansions.insert("ai".to_string(), vec!["artificial intelligence".to_string(), "machine learning".to_string(), "neural networks".to_string()]);
        tech_expansions.insert("nlp".to_string(), vec!["natural language processing".to_string(), "text analysis".to_string()]);
        tech_expansions.insert("api".to_string(), vec!["rest".to_string(), "endpoint".to_string(), "microservice".to_string()]);
        tech_expansions.insert("db".to_string(), vec!["database".to_string(), "sql".to_string(), "storage".to_string()]);
        
        domains.insert("technology".to_string(), tech_expansions);
        
        // Business domain
        let mut business_expansions = HashMap::new();
        business_expansions.insert("roi".to_string(), vec!["return on investment".to_string(), "profitability".to_string()]);
        business_expansions.insert("kpi".to_string(), vec!["key performance indicator".to_string(), "metrics".to_string()]);
        business_expansions.insert("b2b".to_string(), vec!["business to business".to_string(), "enterprise".to_string()]);
        business_expansions.insert("b2c".to_string(), vec!["business to consumer".to_string(), "retail".to_string()]);
...
mod tests {
    ...
    async fn test_synonym_expansion() {
        ...
        let synonym_result = results.iter()
    ...
    async fn test_domain_expansion() {
        ...
        let domain_result = results.iter()
