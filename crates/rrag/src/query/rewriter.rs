...
impl QueryRewriter {
    ...
    fn clarify_query(&self, query: &str) -> Option<String> {
        ...
        if query_lower.starts_with("what is") {
            return Some(format!("{} and how does it work?", query.trim_end_matches('?')));
        }
    ...
    fn normalize_style(&self, query: &str) -> String {
        ...
        normalized = Regex::new(r"[!]{2,}").unwrap()
            .replace_all(&normalized, "!").to_string();
        normalized = Regex::new(r"[?]{2,}").unwrap()
            .replace_all(&normalized, "?").to_string();
        
        // Fix spacing
        normalized = Regex::new(r"\s+").unwrap()
            .replace_all(&normalized, " ").to_string();
    ...
    fn is_question(&self, query: &str) -> bool {
        let question_words = ["what", "how", "why", "when", "where", "who", "which", "can", "is", "are", "do", "does"];
        let query_lower = query.to_lowercase();
        ...
        vec![
            ...
            QueryTransformation {
                ...
                transform: |query| {
                    ...
                    if tech_terms.iter().any(|term| query.to_lowercase().contains(term)) {
...
impl GrammarPattern {
    ...
    fn apply(&self, query: &str) -> Option<String> {
        if self.pattern.is_match(query) {
            Some(self.pattern.replace_all(query, &self.replacement).to_string())
        } else {
            None
        }
    }
...
mod tests {
    ...
    async fn test_query_rewriter() {
        ...
        let grammar_corrected = results.iter()
