...
impl HyDEGenerator {
    ...
    pub async fn generate(&self, query: &str) -> RragResult<Vec<HyDEResult>> {
        ...
        for (i, doc) in hypothetical_docs.iter().enumerate() {
            if doc.len() < self.config.min_document_length || doc.len() > self.config.max_document_length {
                continue;
            }
    ...
    fn generate_hypothetical_documents(
        ...
        for pattern in &self.answer_patterns {
            if pattern.triggers.iter().any(|trigger| 
    ...
    fn detect_query_type(&self, query: &str) -> String {
        ...
        } else if query_lower.contains("compare") || query_lower.contains("vs") || query_lower.contains("difference") {
            "comparative".to_string()
        } else if query_lower.starts_with("when") || query_lower.starts_with("where") || query_lower.starts_with("who") {
            "factual".to_string()
    ...
    fn detect_domain(&self, query: &str) -> Option<String> {
        ...
        let domains = [
            ("technology", vec!["code", "programming", "software", "api", "database", "algorithm", "computer", "tech"]),
            ("science", vec!["research", "study", "experiment", "theory", "analysis", "scientific", "hypothesis"]),
            ("business", vec!["market", "sales", "revenue", "customer", "profit", "strategy", "management", "company"]),
            ("health", vec!["medical", "health", "disease", "treatment", "doctor", "medicine", "patient", "healthcare"]),
            ("education", vec!["learn", "study", "school", "university", "course", "education", "teach", "academic"]),
            ("finance", vec!["money", "investment", "financial", "bank", "trading", "economics", "cost", "price"]),
        ];
    ...
    fn extract_key_terms(&self, query: &str) -> Vec<String> {
        let stop_words = ["the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", "is", "are", "was", "were", "be", "been", "have", "has", "had", "do", "does", "did", "will", "would", "could", "should", "may", "might", "can", "what", "how", "why", "when", "where", "who", "which"];
    ...
    fn calculate_confidence(&self, query: &str, document: &str, query_type: &str) -> f32 {
        ...
        let term_matches = query_terms.iter()
        ...
        match query_type {
            "definitional" if document.contains("is") || document.contains("refers to") => confidence += 0.1,
            "procedural" if document.contains("step") || document.contains("process") => confidence += 0.1,
            "comparative" if document.contains("compare") || document.contains("difference") => confidence += 0.1,
            _ => {}
        }
    ...
    fn init_answer_patterns() -> Vec<AnswerPattern> {
        vec![
            AnswerPattern {
                ...
                triggers: vec!["algorithm".to_string(), "system".to_string(), "technology".to_string()],
            ...
            AnswerPattern {
                ...
                triggers: vec!["research".to_string(), "study".to_string(), "analysis".to_string()],
                generator: |query, _config| {
                    vec![format!(
...
mod tests {
    ...
    async fn test_procedural_query() {
        ...
        assert!(results[0].hypothetical_answer.contains("step") || 
    ...
    async fn test_comparative_query() {
        ...
        let results = hyde.generate("Python vs Rust performance comparison").await.unwrap();
    ...
    fn test_domain_detection() {
        ...
        assert_eq!(hyde.detect_domain("machine learning algorithm"), Some("technology".to_string()));
        assert_eq!(hyde.detect_domain("medical research study"), Some("health".to_string()));
        assert_eq!(hyde.detect_domain("market analysis strategy"), Some("business".to_string()));
